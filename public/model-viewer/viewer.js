...
function getDefaultExportRoot() {
  return window.FAREVER_MODEL_VIEWER_DEFAULT_ROOT || 'model-library';
}
...
function fatal(err) {
  console.error(err);
  setStatus(`Error:\n${err?.stack || err?.message || String(err)}`, true);
}
...
function createLights(scene) {
  const hemi = new THREE.HemisphereLight(0xc8f2ff, 0x5f5242, 0.05);
  scene.add(hemi);
...
function createGround(scene) {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: 0x2d2d2d, roughness: 1, metalness: 0 });
...
function createTextureLoaders() {
  ...
  async function decodeTextureFromArrayBuffer(arrayBuffer, { gradMat = false, isSlot = false } = {}) {
    let tex;
    if (isDDS(arrayBuffer)) {
      tex = ddsLoader.parse(arrayBuffer, true);
    } else {
      tex = await loadBlobTexture(new Blob([arrayBuffer]));
    }
  ...
  async function loadTextureFromArrayBuffer(arrayBuffer, { gradMat = false, isSlot = false, cacheKey = null } = {}) {
    const effectiveKey = cacheKey || `embedded::${gradMat ? 'grad' : 'fallback'}::${isSlot ? 'slot' : 'nonslot'}::${arrayBuffer.byteLength}`;
    if (textureCache.has(effectiveKey)) return textureCache.get(effectiveKey);
    const promise = decodeTextureFromArrayBuffer(arrayBuffer, { gradMat, isSlot });
    textureCache.set(effectiveKey, promise);
    try {
      return await promise;
    } catch (err) {
      textureCache.delete(effectiveKey);
      throw err;
    }
  }
...
function normalizeManifest(manifestRaw) {
  const grad = manifestRaw.gradMat || null;
  ...
  return { raw: manifestRaw, grad, hmdMaterials, hmdByName };
}
...
async function extractEmbeddedProjectData(gltf) {
  ...
  return {
    manifest: normalizeManifest(project.manifest || null),
    embeddedBuffers,
  };
}
...
async function loadOutTexture(relPath, manifestBase, textureLoaders, opts = {}, embeddedBuffers = null) {
  ...
  if (embeddedBuffers && embeddedBuffers.has(relPath)) {
    ...
    return await textureLoaders.loadTextureFromArrayBuffer(entry.arrayBuffer, {
      ...opts,
      cacheKey: `embedded::${relPath}::${opts.gradMat ? 'grad' : 'fallback'}::${opts.isSlot ? 'slot' : 'nonslot'}`,
    });
  }
...
async function applyMaterials(root, manifest, manifestUrl, textureLoaders, mainLight, camera, viewerLighting, embeddedBuffers = null) {
  ...
  const gradResources = await prepareGradMatResources(manifest.grad, manifestBase, textureLoaders, embeddedBuffers);

  traverseMeshes(root, (mesh) => {
    if (gradResources) ensureGradMatAttributes(mesh);
  });

  const lightDir = buildMainLightDir(mainLight);

  const updateShaderUniforms = (shaderMat) => {
    shaderMat.uniforms.uMainLightDir.value.copy(lightDir);
  ...
  traverseMeshes(root, async () => {});

  const meshList = [];
  traverseMeshes(root, (mesh) => meshList.push(mesh));
  ...
  for (const mesh of meshList) {
    ...
    for (let i = 0; i < oldMats.length; i++) {
      ...
      const hmd = manifest.hmdByName.get(srcMat.name) || manifest.hmdMaterials[i] || manifest.hmdMaterials[0] || null;
      if (hmd?.diffuseOut) {
...
async function loadGltf(gltfUrl) {
...
function buildCatalogEntries(summary) {
  ...
  return exportsList.map((item) => {
    ...
    const label = labelSource.replace(/\.(glb|gltf)$/i, '');
    return {
      key,
      label,
      glbRel,
...
function renderModelList(entries, selectedKey, onSelect) {
  ...
  for (const entry of entries) {
    ...
    if (entry.warnings.length) {
      ...
      meta.textContent = entry.warnings[0];
...
function updateBrowserUrl({ root, modelKey, viewerLighting }) {
  const params = new URLSearchParams();
  if (root) params.set('root', root);
  if (modelKey) params.set('model', modelKey);
  if (viewerLighting.mainLightIntensity !== 0.12) params.set('mainLightIntensity', String(viewerLighting.mainLightIntensity));
  if (viewerLighting.mainLightPower !== 1.0) params.set('mainLightPower', String(viewerLighting.mainLightPower));
  if (viewerLighting.shadow !== 1.0) params.set('shadow', String(viewerLighting.shadow));
...
async function main() {
  ...
  const viewerLighting = {
    mainLightIntensity: getOptionalNumberParam('mainLightIntensity', 0.12),
    mainLightPower: getOptionalNumberParam('mainLightPower', 1.0),
    shadow: getOptionalNumberParam('shadow', 1.0),
  ...
  async function loadResolvedModel({ modelUrl, manifestUrl = null, label, rootValue = null, modelKey = null, isGlb = false }) {
    ...
    if (isGlb) {
      ...
      if (!embedded.manifest) {
        throw new Error('GLB loaded but no embedded prefabToGltf manifest was found.');
      }
    ...
    setStatus(
      `Loaded: ${label || pathBasenameNoExt(modelUrl)}\n\n` +
      `GradMat fixes:\n` +
      `- TEXCOORD_0 used for slot/ramp lookup\n` +
      `- TEXCOORD_1 used for lines/pattern/marks\n` +
      `- GradMat textures sampled in raw space (not sRGB)\n` +
      `- sparse slot indices preserved\n` +
      `- embedded GLB textures + manifest supported\n\n` +
      `Viewer light defaults are approximate.`
    );

    if (rootValue && modelKey) {
      updateBrowserUrl({ root: rootValue, modelKey, viewerLighting });
    }
  }

  async function handleEntrySelect(entry, forcedRootValue = null, forcedRootBaseUrl = null) {
    if (!entry) return;
    const rootValue = forcedRootValue || exportRootInput?.value || localStorage.getItem('prefabViewer.exportRoot') || getDefaultExportRoot();
    ...
    if (entry.glbRel) {
      await loadResolvedModel({
        modelUrl: resolveUrl(entry.glbRel, rootBaseUrl),
        label: entry.label,
        rootValue,
        modelKey: entry.key,
        isGlb: true
      ...
      await loadResolvedModel({
        modelUrl: resolveUrl(entry.gltfRel, rootBaseUrl),
        manifestUrl: resolveUrl(entry.manifestRel, rootBaseUrl),
        label: entry.label,
        rootValue,
        modelKey: entry.key,
        isGlb: false
  ...
  async function loadCatalog(rootValue, preferredModelKey = null) {
    const normalizedRoot = String(rootValue || '').trim() || getDefaultExportRoot();
    ...
    if (preferredModelKey) {
      entry = catalogEntries.find((item) => item.key === preferredModelKey || item.label === preferredModelKey) || null;
    }
  ...
  refreshButton?.addEventListener('click', async () => {
    try {
      await loadCatalog(exportRootInput?.value || getDefaultExportRoot(), selectedKey);
    } catch (err) {
      console.error(err);
      setCatalogStatus(`Failed to load export summary: ${err.message || err}`, true);
    }
  });
  ...
  try {
    const rememberedRoot = params.get('root') || localStorage.getItem('prefabViewer.exportRoot') || getDefaultExportRoot();
    if (params.get('root') || (!directGlbParam && !directGltfParam)) {
      ...
      await loadResolvedModel({
        modelUrl: resolveUrl(directGlbParam || directGltfParam, window.location.href),
        manifestUrl: directManifestParam ? resolveUrl(directManifestParam, window.location.href) : null,
        label: pathBasenameNoExt(directGlbParam || directGltfParam),
        isGlb: !!directGlbParam
      });
    ...
    console.error(err);
    setCatalogStatus(
      `Failed to load prefab-export-summary.json.\nSet the export root in the sidebar and click Reload.\n\nStatic hosting still works with npx serve . as long as the exported folder is inside the served directory.`,
      true
    );
