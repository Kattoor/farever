...
function setStatus(text, isError = false) {
  if (isError) setCatalogStatus(String(text || ''), true);
}
...
function applyPartSelection(root, partEntries, selectedPartPath, camera, controls) {
  ...
  fitCameraToObject(camera, controls, target || root);
}
...
async function main() {
  ...
  partSelect?.addEventListener('change', () => {
    ...
    if (currentRoot) {
      applyPartSelection(currentRoot, currentPartEntries, currentSelectedPart, camera, controls);
    }
  });
  ...
  async function loadResolvedModel({ modelUrl, manifestUrl = null, label, rootValue = null, modelKey = null, isGlb = false }) {
    ...
    applyPartSelection(root, currentPartEntries, currentSelectedPart, camera, controls);
